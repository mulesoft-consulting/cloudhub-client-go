/*
Deployment

The Application Manager API exists to provide Mule Application management operations from Anypoint Control Planes to any Runtime Plane, with capabilities that include:   - Deploying a Mule Application or API to a Mule Runtime   - Scaling up or down a running application   - Managing application settings (ie: properties)   - Deleting a Mule Application Deployment   - Changing artifact version or configurations of a deployment   - Starting, Stopping or Restarting applications   - etc. This API currently supports deployments to Runtime Fabric and CloudHub 2.0 targets only. 

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package application_manager_v2

import (
	"encoding/json"
)

// checks if the ResourcesMemory type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ResourcesMemory{}

// ResourcesMemory struct for ResourcesMemory
type ResourcesMemory struct {
	Limit *string `json:"limit,omitempty"`
	Reserved *string `json:"reserved,omitempty"`
}

// NewResourcesMemory instantiates a new ResourcesMemory object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewResourcesMemory() *ResourcesMemory {
	this := ResourcesMemory{}
	return &this
}

// NewResourcesMemoryWithDefaults instantiates a new ResourcesMemory object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewResourcesMemoryWithDefaults() *ResourcesMemory {
	this := ResourcesMemory{}
	return &this
}

// GetLimit returns the Limit field value if set, zero value otherwise.
func (o *ResourcesMemory) GetLimit() string {
	if o == nil || IsNil(o.Limit) {
		var ret string
		return ret
	}
	return *o.Limit
}

// GetLimitOk returns a tuple with the Limit field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ResourcesMemory) GetLimitOk() (*string, bool) {
	if o == nil || IsNil(o.Limit) {
		return nil, false
	}
	return o.Limit, true
}

// HasLimit returns a boolean if a field has been set.
func (o *ResourcesMemory) HasLimit() bool {
	if o != nil && !IsNil(o.Limit) {
		return true
	}

	return false
}

// SetLimit gets a reference to the given string and assigns it to the Limit field.
func (o *ResourcesMemory) SetLimit(v string) {
	o.Limit = &v
}

// GetReserved returns the Reserved field value if set, zero value otherwise.
func (o *ResourcesMemory) GetReserved() string {
	if o == nil || IsNil(o.Reserved) {
		var ret string
		return ret
	}
	return *o.Reserved
}

// GetReservedOk returns a tuple with the Reserved field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ResourcesMemory) GetReservedOk() (*string, bool) {
	if o == nil || IsNil(o.Reserved) {
		return nil, false
	}
	return o.Reserved, true
}

// HasReserved returns a boolean if a field has been set.
func (o *ResourcesMemory) HasReserved() bool {
	if o != nil && !IsNil(o.Reserved) {
		return true
	}

	return false
}

// SetReserved gets a reference to the given string and assigns it to the Reserved field.
func (o *ResourcesMemory) SetReserved(v string) {
	o.Reserved = &v
}

func (o ResourcesMemory) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ResourcesMemory) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Limit) {
		toSerialize["limit"] = o.Limit
	}
	if !IsNil(o.Reserved) {
		toSerialize["reserved"] = o.Reserved
	}
	return toSerialize, nil
}

type NullableResourcesMemory struct {
	value *ResourcesMemory
	isSet bool
}

func (v NullableResourcesMemory) Get() *ResourcesMemory {
	return v.value
}

func (v *NullableResourcesMemory) Set(val *ResourcesMemory) {
	v.value = val
	v.isSet = true
}

func (v NullableResourcesMemory) IsSet() bool {
	return v.isSet
}

func (v *NullableResourcesMemory) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableResourcesMemory(val *ResourcesMemory) *NullableResourcesMemory {
	return &NullableResourcesMemory{value: val, isSet: true}
}

func (v NullableResourcesMemory) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableResourcesMemory) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


